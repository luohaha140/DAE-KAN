...
def exact_solution(t_sim):
    ...
    y4 = np.cos(t_sim)
    y5 = 1 + np.sin(2 * t_sim)
    return np.array([y1, y2, y3, y4, y5], dtype=np.float64)
...
def train():
    ...
    # 注意：Adam优化器被创建但未在训练步骤中使用
    optimizer_1 = torch.optim.Adam(model.parameters(), lr=1e-6)
...
if __name__ == "__main__":
    # 加载预训练模型
    m_state_dict = torch.load("../logs/model_particle_index1.pth")
    model = model_KAN(
        width_dyn=[1, 5, 5, 4],
        width_alg=[1, 5, 5, 1],
        grid_dyn=6,
    ...
    # 保存模型
    # torch.save(model.state_dict(), "../logs/model_particle_index1.pth")
