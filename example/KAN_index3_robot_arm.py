...
def DAE_loss(y_n):
    ...
    # 提取预测变量
    u_1, u_2, v_1, v_2, d = y_pred[:, 0], y_pred[:, 1], y_pred[:, 2], y_pred[:, 3], y_pred[:, 4]
    ...
    f1 = u_1_t - v_1
    f2 = u_2_t - v_2
    f3 = v_1_t * (5 + 3 * torch.cos(u_2)) + v_2_t * (1 + 1.5 * torch.cos(u_2)) + \
         (d - v_1) * (torch.cos(u_1) + torch.cos(u_1 + u_2)) + 3 * u_1
    f4 = v_1_t * (1 + 1.5 * torch.cos(u_2)) + v_2_t + (d - v_1) * torch.cos(u_1 + u_2) - \
         u_1 * (1 - 1.5 * torch.cos(u_2))
    f5 = torch.sin(u_1) + torch.sin(u_1 + u_2)
    ...
    # 初始条件损失
    x_init_loss = torch.mean((u_1[x_0] - 0) ** 2)
    y_init_loss = torch.mean((u_2[y_0] - 0) ** 2)
    u_init_loss = torch.mean((v_1[u_0] - 1) ** 2)
    v_init_loss = torch.mean((v_2[v_0] - (-2)) ** 2)
    d_init_loss = torch.mean((d[d_0] - 1) ** 2)
    init_loss = x_init_loss + y_init_loss + u_init_loss + v_init_loss + d_init_loss

    # 总损失
    total_loss = 1 * F1 + 1 * F2 + 1 * F3 + 1 * F4 + 1 * F5 + init_loss
    return total_loss
...
if __name__ == "__main__":
    ...
    # 加载预训练模型
    m_state_dict = torch.load("../logs/model_robot_arm_index3.pth")
    model = model_KAN(
        width_dyn=[1, 4, 4, 4],
        width_alg=[1, 2, 2, 1],
        grid_dyn=5,
    ...
    # 保存模型
    # torch.save(model.state_dict(), "../logs/model_robot_arm_index3_float64_test.pth")
